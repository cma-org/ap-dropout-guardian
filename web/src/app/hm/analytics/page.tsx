...
export default function HMAnalyticsPage() {
  ...
  const handleDownloadReport = async () => {
    const doc = new jsPDF();
    const title = "School Analytics Report";
    ...
    doc.save(`School_Report_${new Date().getTime()}.pdf`);
  };
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 mt-1 max-w-2xl">
            {lang === "en" 
              ? "Comprehensive multi-dimensional analysis of dropout risk factors across all grades and demographics."
              : "అన్ని తరగతులు మరియు జనాభా వివరాల ప్రకారం డ్రాపౌట్ ప్రమాద కారకాల సమగ్ర బహుళ-డైమెన్షనల్ విశ్లేషణ."}
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            {lang === "en" ? "Export Summary" : "సారాంశాన్ని ఎగుమతి చేయండి"}
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div id="chart-factors" className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm flex flex-col h-[400px]">
          <div className="mb-6">
            ...
            <p className="text-xs text-zinc-500 mt-1">Key indicators contributing to dropout risk across the school.</p>
          </div>
          <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div id="chart-grade-risk" className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm flex flex-col h-[400px]">
          <div className="mb-6">
            ...
            <p className="text-xs text-zinc-500 mt-1">High and critical risk students compared to total enrollment per grade.</p>
          </div>
          <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div id="chart-attendance-trend" className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm flex flex-col h-[400px]">
          <div className="mb-6 flex justify-between items-start">
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-1">Inverse correlation between monthly attendance and school-wide risk.</p>
            </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center">
          <div className="flex-1">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
              ...
              {lang === "en" ? "Strategic Analytics Summary" : "వ్యూహాత్మక విశ్లేషణ సారాంశం"}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-blue-50/80">
              <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm border border-white/10">
                <p className="font-bold text-white mb-1">{lang === "en" ? "Seasonal Alert" : "కాలానుగుణ హెచ్చరిక"}</p>
                <p>{lang === "en" 
                  ? "A 12% spike in risk is expected next month due to local harvest. Proactive parent engagement is recommended." 
                  : "స్థానిక కోతల కారణంగా వచ్చే నెలలో ప్రమాదం 12% పెరుగుతుందని అంచనా. తల్లిదండ్రులతో ముందస్తు సంప్రదింపులు సూచించబడతాయి."}</p>
              </div>
              <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm border border-white/10">
                <p className="font-bold text-white mb-1">{lang === "en" ? "Gender Insight" : "లింగం వారీగా విశ్లేషణ"}</p>
                <p>{lang === "en" 
                  ? "Female students show higher attendance resilience but face 8% higher risk related to transport issues." 
                  : "బాలికలు మెరుగైన హాజరును ప్రదర్శిస్తున్నారు, కానీ రవాణా సమస్యల వల్ల 8% ఎక్కువ ప్రమాదాన్ని ఎదుర్కొంటున్నారు."}</p>
              </div>
            </div>
          </div>
          <button 
